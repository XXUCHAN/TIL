<script lang="ts">
	...
	function deleteTodo(id: number){
		const newTodo = JSON.parse(localStorage.getItem('todos') || '[]').filter(todo => todo.id !== id);
		localStorage.setItem('todos', JSON.stringify(newTodo));
		todos.set(newTodo);
	}
	function deleteCompletedTodo(id: number){
		const newTodo = JSON.parse(localStorage.getItem('completedTodos') || '[]').filter(todo => todo.id !== id);
